...
import { normalizeForm, getIconKey } from './medication-forms';
  ...
  return {
    ...
    async getByCode(params: {
      ...
      if (params.cis || params.externalId) {
        ...
        const cis = params.cis ?? params.externalId!;
        url = `${baseUrl}/v1/medicaments/${encodeURIComponent(cis)}`;
      } else {
        // CIP lookup — search by CIP param
        url = `${baseUrl}/v1/medicaments?cip=${encodeURIComponent(params.cip!)}`;
      }
