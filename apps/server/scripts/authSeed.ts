...
const SEED_MEDICATIONS = [
  ...
  {
    name: 'LÉVOTHYROXINE 75µg',
    form: 'comprimé',
    cis: '65678500',
    substances: ['LÉVOTHYROXINE'],
    dosage: '75µg',
  },
  {
...
async function seedPillbox(patientIds: string[]) {
  ...
  for (const patientId of patientIds) {
    ...
    for (const med of meds) {
      ...
      // Create 1-3 schedules per medication
      ...
      const scheduleRows = moments.map((moment) => ({
        ...
        unit: med.form === 'gélule' ? 'gélule' : 'comprimé',
      }));
